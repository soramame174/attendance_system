...
public class AttendanceServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if ("bulk_delete".equals(action) && "admin".equals(user.getRole())) {
		    ...
		    if (recordsToDelete != null && recordsToDelete.length > 0) {
		        if (attendanceDAO.deleteBulkAttendance(recordsToDelete)) {
			        session.setAttribute("successMessage", "選択した勤怠記録を削除しました。");
		        } else {
		        	session.setAttribute("errorMessage", "選択した勤怠記録の削除に失敗しました。");
		        }
			}
		}
